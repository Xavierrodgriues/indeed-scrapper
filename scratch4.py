...
pattern = re.compile(
    r'(?:[Mm]inimum\s+(?:of\s+)?)?((?:\d+|one|two|three|four|five|six|seven|eight|nine|ten|eleven|twelve)\+?\s*(?:to|-)?\s*(?:\d+|one|two|three|four|five|six|seven|eight|nine|ten|eleven|twelve)?\+?)\s*years?(?:\'s)?\s*(?:of\s*)?(?:hands-on\s*)?(?:professional\s*)?(?:industry\s*)?(?:related\s*)?(?:relevant\s*)?experience(?:\s+(?:in|with|using|developing|on|working with)\s+([a-zA-Z0-9_\-\s]+))?',
    re.IGNORECASE
)
...
for job in data:
    ...
    for m in matches:
        ...
        if context:
            ...
            # Truncate to first 5 words max
            words = ctx_clean.split()
            if len(words) > 5:
                ctx_clean = " ".join(words[:5]) + "..."
            if ctx_clean:
                val += f" ({ctx_clean})"
        results.append(val)
